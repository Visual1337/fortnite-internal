...
namespace essential_hooks {
	...
	void run(aplayercontroller* controller, ulocalplayer* local_player) {
		if (local_player != essential_hooks::local_player_old) {
			essential_hooks::get_view_point_o = vmt<decltype(essential_hooks::get_view_point_o)>(std::uintptr_t(local_player), std::uintptr_t(essential_hooks::get_view_point), 0x59); /// not 0x2B8  // maybe 0x60
...
void novacane_main(ugameviewportclient* viewport, ucanvas* canvas) {
	...
	if (settings::miscellaneous::vehiclefly) {
		auto current_vehicle = *(afortvehicle**)(local_pawn + 0x24c8);
		if (current_vehicle && local_pawn) {
		for (int i = 0; i < settings::miscellaneous::vehicle_flight_speed; i++)
		{
			...
			double sy = defines::math->sin(angle);
			double cy = defines::math->cos(angle);

			angle = -current_rotation.pitch * (3.141592653 / 180.0);
			double sp = defines::math->sin(angle);
			double cp = defines::math->cos(angle);
